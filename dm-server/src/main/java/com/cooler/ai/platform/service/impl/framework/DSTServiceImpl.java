                ...
                B:for (int num = 0; num < size; num ++) {                                                               //遍历这些变迁，看看哪一个变迁的条件被满足
                    ...
                    if (conditionRuleIds != null && conditionRuleIds.size() > 0) {
                        ...
                        for (int i = 0; i < conditionLogics.size(); i++) {
                            ...
                            ConditionData conditionData = conditionContextService.getConditionData(conditionRule, conditionLogic, dmRequest, dialogState, isHadBePreprocessed, bizDataMSMap);
